...
&nbsp;&nbsp;신경망의 좋은 성능 달성을 위해서는 적절한 초기화가 필요하다. 양자화된 네트워크에 대해서도 마찬가지다. 선행 연구들에서는 사전 훈련된 신경망을 이용해 초기화 한다. 그런나 $$\sgn$$의 출력신호는 ReLU 계층의 출력과 매우 다름으로 이러한 방법을 사용하면 치명정인 정확도 손실이 발생한다.
